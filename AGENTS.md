...
**`Redact` and `IsMultiStatement` scan literals in opposite directions, and neither may be collapsed to one pass.** Dialects disagree about whether `\'` closes a literal and whether `$$` opens one, so both functions run the lexer under more than one reading. `Redact` must never leave a literal byte in its output, so it takes the **union** of the readings and over-redacts ambiguous SQL. `IsMultiStatement` must never miss a `;`, so it takes the opposite side: a separator counts when **any** reading leaves it outside a literal, because each reading of `$$` is blind to a payload the other catches (`UPDATE t AS $$ SET id = 1; DROP TABLE t` vs `SELECT $$'$$; DROP TABLE t`). Collapsing either function to a single reading is a literal leak or an EXPLAIN separator bypass — both happened during development and are pinned by `TestRedactNoLeakAcrossDialectAmbiguity` and `TestIsMultiStatementNeedsBothReadings`. Over-redaction also changes the fingerprint, so an ambiguous query groups separately for N+1/dedup; that is the accepted cost.
