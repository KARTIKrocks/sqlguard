...
sqlguard is pre-1.0 and its nine Go modules are tagged in lockstep, so one
version number describes the whole project. Versioned docs get unmaintainable
fast if every release snapshots the whole tree, so this site follows five
rules.
...
broke it?* If nothing broke, the snapshot is a byte-identical copy of twenty
...
`0.2` is the first snapshot, cut when the site launched, so that `/docs/`
serves a released version from day one. The next one is cut when a release
changes something a `0.2` reader relies on — say a rule's default severity or
the shape of `Result` — not when a release merely adds a rule (see rule 2).

Go makes this rule stronger than it would be elsewhere. While the project is
`0.x`, minors may break; once it is `1.x`, semver plus the Go compatibility
promise means additive minors leave the documented contract intact, and a
`v2` is a different import path (`github.com/KARTIKrocks/sqlguard/v2`) with
its own pkg.go.dev — a genuinely different package. **Always snapshot at a
major.**
...
is a cheap one, and it is a better answer — a `0.2` snapshot tells you what
...
| Situation | Write |
| --- | --- |
| New row in an API table | append `_0.3+_` to the description cell |
| New option or behaviour in prose | open the paragraph with `_Added in 0.3._` |
| New member inside a code block | trailing `// 0.3+` comment |
| Behaviour that changed | `_Changed in 0.3._` plus one line on what it was before |

Markers use `MAJOR.MINOR` — `0.3`, not `v0.3.0` — so they match snapshot names
...
Versions are `0.2`, `0.3`, `1.0`. Never `0.2.0`, never `v0.2`.

A patch release (`0.2.1`) that changes documented behaviour is **edited into the
existing `versioned_docs/version-0.2/` in place**. It does not get its own
...
`maxLiveVersions` in `versions.config.json` caps how many snapshots get built
and indexed. Older ones stay in git — readable at their tag, restorable by
bumping the number — but they don't cost build time or search index size.

This keeps build time flat as releases accumulate instead of growing linearly.

Four is generous under rule 1. Snapshotting every minor would burn the whole
window quickly; snapshotting only breaking ones makes it last years. Raise it
only if the reason is real support obligations, not release count.
...
| `versioned_docs/version-0.2/` | the current release | `/docs/` |
...
APIs carry their `_0.3+_` markers.
...
```bash
cd website
npm run cut-version -- 0.3
npm run check
```

`versions.json`, `versioned_docs/version-0.3/`, and
`versioned_sidebars/version-0.3-sidebars.json` are created for you, `/docs/`
starts serving 0.3, and 0.2 moves into the version dropdown.
...
on [pkg.go.dev](https://pkg.go.dev/github.com/KARTIKrocks/sqlguard) and are
always correct; hand-copying them here creates a second source of truth that
drifts.
