...
:::warning Known issue in 0.2
The `v0.2.0` CLI binary ships without a database driver, so `sqlguard explain`
fails with `sql: unknown driver "postgres" (forgotten import?)` for every
DSN. The `explain` package works with any `*sql.DB` you open yourself. Fixed
on `main` (the CLI now links `pgx/v5/stdlib` and `go-sql-driver/mysql`) and
in the next release — see the [Next](/docs/next/explain#drivers) docs.
:::
